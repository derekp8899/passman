...
User master password will not be stored will compare to stored SHA2-512 hashes which combine the master password hash with the random
autogenerated secret key which will be stored, future possible upgrade to SHA-3
...
char* keyGenHKDF(){
  ...
  EVP_PKEY_CTX *pctx;
  unsigned char *out=malloc(sizeof(unsigned char)*256);
  size_t outlen = 256;
  ...
  if (EVP_PKEY_CTX_set1_hkdf_salt(pctx, "salt", 4) <= 0)
    /* Error */
    return;
  if (EVP_PKEY_CTX_set1_hkdf_key(pctx, "sacret", 6) <= 0)
    /* Error */
    return;
